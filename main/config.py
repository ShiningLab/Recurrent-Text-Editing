...
class Config():
      ...
      def __init__(self): 
        ...
        self.TASK_PATH = os.path.join(self.CURR_PATH, 'res/data/', 
            self.method, 'vocab_size_{}'.format(self.vocab_size), 
            'seq_len_{}'.format(self.seq_len), 'data_size_{}'.format(self.data_size))
        self.DATA_PATH = os.path.join(self.TASK_PATH, 'data.json')
        self.VOCAB_PATH = os.path.join(self.TASK_PATH, 'vocab.json')
        # initialization
        self.pad_symbol = '<pad>'
        self.start_symbol = '<s>'
        self.end_symbol = '</s>'
