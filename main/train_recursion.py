...
class TextEditor(object):
    ...
    def offline_recursion_collate_fn(self, data): 
        ...
        def preprocess(xs, ys): 
            # vocab to index
            xs = [vocab_to_index(x, self.src_vocab2idx_dict) for x in xs]
            ys = [vocab_to_index(y, self.src_vocab2idx_dict) for y in ys]
    ...
    def online_recursion_collate_fn(self, data): 
        ...
        def preprocess(xs, ys): 
            # vocab to index
            xs = [vocab_to_index(x, self.src_vocab2idx_dict) for x in xs]
            ys = [vocab_to_index(y, self.tgt_vocab2idx_dict) for y in ys]
            # add start and end symbol 
            xs = [torch.Tensor(x + [self.config.end_idx]) for x in xs]
            ys = [torch.Tensor(y + [self.config.end_idx]) for y in ys] 
            return xs, ys
    ...
    def load_data(self): 
        ...
        self.testset_generator = torch_data.DataLoader(
              self.test_dataset, 
              batch_size=self.config.batch_size, 
              collate_fn=self.offline_recursion_collate_fn, 
              shuffle=False,
              drop_last=False)
    ...
    def setup_model(self): 
        ...
        self.model = pick_model(self.config.model_name, "end2end", self.config)
    ...
    # def test(self):
    #     print('\nTesting...')
    #     model = pick_model(self.config.model_name, "recursion", self.config)
    #     checkpoint_to_load =  torch.load(self.config.LOAD_POINT, map_location=self.config.device) 
    ...
    #             # print(index_to_vocab(xs.cpu().detach().numpy()[0], self.src_idx2vocab_dict))
    #             # print(index_to_vocab(ys.cpu().detach().numpy()[0], self.src_idx2vocab_dict))
    #             ys_ = model(xs)
    #             xs = xs.cpu().detach().numpy() # batch_size, max_xs_seq_len
    #             ys = ys.cpu().detach().numpy() # batch_size, max_ys_seq_len
    #             ys_ = torch.argmax(ys_, dim=2).cpu().detach().numpy() # batch_size, max_ys_seq_len
    ...
    #     all_xs = [vocab_to_index(x, self.src_idx2vocab_dict) for x in all_xs]
    #     all_ys = [vocab_to_index(y, self.src_idx2vocab_dict) for y in all_ys]
    #     all_ys_ = [vocab_to_index(y, self.tgt_idx2vocab_dict) for y in all_ys_]
