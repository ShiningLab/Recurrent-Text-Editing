...
class TextEditor(object):
    ...
    def test_tagging_collate_fn(self, data): 
        # a customized collate function used in the data loader 
        data.sort(key=len, reverse=True)
        xs, ys = zip(*data)
        xs, ys = preprocess(
        ...
        return xs, torch.Tensor(x_lens), ys
    ...
    def load_data(self): 
        ...
        self.trainset_generator = torch_data.DataLoader(
              ...
              collate_fn=self.train_tagging_collate_fn, 
              shuffle=self.config.shuffle, 
              num_workers=self.config.num_workers, 
              pin_memory=self.config.pin_memory,
              drop_last=self.config.drop_last)
        # val data loader
        self.val_dataset = OfflineDataset(self.data_dict['val'])
        self.valset_generator = torch_data.DataLoader(
              self.val_dataset, 
              batch_size=self.config.batch_size, 
              collate_fn=self.test_tagging_collate_fn, 
              shuffle=False, 
              num_workers=self.config.num_workers, 
              pin_memory=self.config.pin_memory,
              drop_last=False)
        # test data loader
        self.test_dataset = OfflineDataset(self.data_dict['test'])
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in valset_generator:
                data = (d.to(self.config.device) for d in data)
                xs, x_lens, ys = data
                ys_ = self.model(xs, x_lens, ys, self.config.tgt_seq_len, 0.)
                xs = xs.cpu().detach().numpy() # batch_size, max_xs_seq_len
                ys = ys.cpu().detach().numpy() # batch_size, max_ys_seq_len
                ys_ = torch.argmax(ys_, dim=2).cpu().detach().numpy() # batch_size, max_ys_seq_len
                xs, ys, ys_ = post_process(xs, ys, ys_, self.config)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for data in testset_generator:
                data = (d.to(self.config.device) for d in data)
                xs, x_lens, ys = data
                ys_ = model(xs, x_lens, ys, self.config.tgt_seq_len, 0.)
                xs = xs.cpu().detach().numpy() # batch_size, max_xs_seq_len
                ys = ys.cpu().detach().numpy() # batch_size, max_ys_seq_len
                ys_ = torch.argmax(ys_, dim=2).cpu().detach().numpy() # batch_size, max_ys_seq_len
                xs, ys, ys_ = post_process(xs, ys, ys_, self.config)
